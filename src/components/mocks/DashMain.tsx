  ...
  return (
    <main className="min-w-0 flex-1 p-5">
      ...
      <div className="mt-3 flex items-center gap-4">
        {TABS.map(([l, n], i) => (<span key={l} className={`flex items-center gap-1.5 text-[13px] font-medium ${i === 0 ? "text-accent" : "text-muted"}`}>{l} <span className={`rounded-full px-1.5 text-[10px] ${i === 0 ? "bg-accent text-white" : "bg-black/5 text-muted"}`}>{n}</span></span>))}
      </div>
