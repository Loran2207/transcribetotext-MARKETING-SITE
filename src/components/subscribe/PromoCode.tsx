...
export function PromoCode() {
  ...
  return (
    <div className="mx-auto mt-8 max-w-md">
      <div className={`flex items-center gap-2 rounded-2xl border p-2 pl-4 transition-colors ${applied ? "border-accent/40 bg-accent-soft" : "border-border bg-surface-soft"}`}>
        <Tag size={16} className={applied ? "shrink-0 text-accent" : "shrink-0 text-muted"} />
        <input
          value={code}
        ...
        />
        {applied ? (
          <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-accent px-3 py-1.5 text-xs font-semibold text-white"><Check size={13} /> Applied</span>
        ) : (
      ...
      {applied && <p className="mt-2 text-center text-xs font-medium text-accent">{subscribe.promo.label}</p>}
    </div>
  );
}
